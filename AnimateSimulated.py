...
# In[4]:
for beta_val in [50, 300, 600]:
    for k_inhib in [2.0, 3.0, 5.0]:
        for sigma in [0.5, 1.0, 1.5, 2.0, 2.5, 3.0]:
            ...
            for data_entry in data:
                neuron_act.append(
                    attr_layer.forward_pass_visualization(
                        data_entry.flatten(), number_of_passes=4
                    )[3]
                )
            ...
            anim.save(
                f"animations/simulated_k_{int(K_INHIB)}_beta_{int(BETA)}_sigma_{int(SIGMA*10)}_pass_4_addition.gif",
                fps=10,
            )
